...
export function CardsPage({ token, rows, onRefresh }: CardsPageProps) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      ["CardID", "AccountID", "CardType", "Status"].some((key) =>
        String(row[key] ?? "")
          .toLowerCase()
          .includes(normalized)
      )
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      await apiRequest("/api/cards/issue", {
        method: "POST",
        token,
        body: formData
      });
    ...
    <>
      <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)] gap-4">
        <Panel className="flex min-h-[102px] flex-col gap-3 px-5 py-4 md:grid md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
          <div className="flex justify-start">
            <div className="flex h-12 w-full max-w-[640px] items-center justify-center rounded-2xl bg-brand-cream px-8">
              <p className="text-center text-lg font-semibold uppercase tracking-[0.26em] text-brand-red">Quản lý thẻ ngân hàng</p>
            </div>
          </div>
        ...
        <Panel className="grid min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden px-5 py-4">
          <div>
            <h3 className="font-display text-2xl text-brand-ink">Quản lý thẻ ngân hàng</h3>
            <p className="mt-1 text-sm text-brand-ink/60">Tìm kiếm và theo dõi trạng thái thẻ trong cùng một khung làm việc rõ ràng.</p>
          </div>
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-brand-ink/20 p-6 backdrop-blur-sm">
          <Panel className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-[32px] px-6 py-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm uppercase tracking-[0.24em] text-brand-red">Phát hành thẻ</p>
                <h3 className="mt-2 font-display text-3xl text-brand-ink">Tạo mới thẻ ngân hàng</h3>
              </div>
              <button
